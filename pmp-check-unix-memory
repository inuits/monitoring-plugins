   ...
   if free -m > "${TEMP}" ; then
      ...
      if ps -eo pid,vsz,ucomm > "${TEMP}" ; then
         ...
         if [ "${PID_PCT:-0}" -ge "${OPT_CRIT}" ]; then
            EXITSTATUS=$STATE_CRITICAL
         elif [ $EXITSTATUS != $STATE_CRITICAL -a "${PID_PCT:-0}" -ge "${OPT_WARN}" ]; then
      ...
      if [ $EXITSTATUS = $STATE_CRITICAL ]; then
         NOTE="CRIT $NOTE"
      elif [ $EXITSTATUS = $STATE_WARNING ]; then
         NOTE="WARN $NOTE"
      else
         NOTE="OK $NOTE"
      fi
...
second is when any single process uses too much memory.  Both checks are
compared to the same thresholds.
...
Percona Monitoring Plugins pmp-check-unix-memory 0.9.0
